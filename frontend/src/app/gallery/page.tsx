...
const ImageCarousel = ({
  ...
  const getPositionClass = (index: number) => {
    if (index === currentIndex)
      return "translate-x-0 scale-100 opacity-100 z-10"; // Current image
            ...
            {images.map((image, index) => (
              ...
              >
                <div className="relative w-[280px] sm:w-[400px] md:w-[600px] h-[200px] sm:h-[300px] md:h-[400px] rounded-2xl overflow-hidden shadow-lg">
                  <Image
                    src={`https://makerspace.iiti.ac.in${image.image}`}
                    alt="Gallery image"
                    fill
...
export default function GalleryPage() {
  ...
  const fetchEvents = async () => {
    try {
      const response = await fetch("https://makerspace.iiti.ac.in/backend/api/events/");
      if (!response.ok) {
  ...
  const handleEditEvent = async () => {
    ...
    try {
      const response = await fetch(
        `https://makerspace.iiti.ac.in/backend/api/events/${selectedEventId}/`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-20 py-8">
              ...
              <CardContent className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                <div className="aspect-video relative overflow-hidden rounded-md">
                  {event.images && event.images.length > 0 ? (
                    <Image
                      src={`https://makerspace.iiti.ac.in/${event.images[0].image}`}
                      alt={event.title}
                      fill
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          <DialogContent
            className="sm:max-w-[90%] md:max-w-[600px] max-h-[80vh] sm:max-h-[90vh]"
