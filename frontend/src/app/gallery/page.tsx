            ...
            {images.map((image, index) => (
              ...
              >
                <div className="relative w-[280px] sm:w-[400px] md:w-[600px] h-[200px] sm:h-[300px] md:h-[400px] rounded-2xl overflow-hidden shadow-lg">
                  <Image
                    src={`http://makerspace.iiti.ac.in${image.image}`}
                    alt="Gallery image"
                    fill
...
export default function GalleryPage() {
  ...
  const fetchEvents = async () => {
    try {
      const response = await fetch("http://makerspace.iiti.ac.in/backend/api/events/");
      if (!response.ok) {
  ...
  const handleEditEvent = async () => {
    ...
    try {
      const response = await fetch(
        `http://makerspace.iiti.ac.in/backend/api/events/${selectedEventId}/`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
              ...
              <CardContent className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                <div className="aspect-video relative overflow-hidden rounded-md">
                  {event.images && event.images.length > 0 ? (
                    <Image
                      src={`http://makerspace.iiti.ac.in/${event.images[0].image}`}
                      alt={event.title}
                      fill
